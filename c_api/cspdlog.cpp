...
extern "C"
{
	...
	clogger* cspd_daily_logger_mt(const char* logger_name, const char* filename, int hour, int minute, bool force_flush){
		clogger *c = alloc_clogger();
	       	if(c != NULL){	
			c->logger = spdlog::daily_logger_mt(logger_name, filename, hour, minute,force_flush);
		}
		return c;
	}
	...
	static inline bool cspd_convert_level(clevel_enum level, spdlog::level::level_enum& level_spd){
	...
	static inline void cspd_printf(char **msg, std::shared_ptr<spdlog::logger> logger, 
